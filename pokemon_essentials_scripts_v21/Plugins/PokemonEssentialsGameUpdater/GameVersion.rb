module PokeUpdater
	...
	def get_poke_updater_text(text_name, variable=nil)
		...
		return default_text(text_name, variable)
	end

	def default_text(text_name, variable=nil)
		case text_name
		when 'NEW_VERSION'
	...
	def validate_version(url, from_update_button=false, update=true)
		begin
			...
			pbMessage(get_poke_updater_text("NO_NEW_VERSION_OR_INTERNET").to_s)
			return
		end
		if data && !data.empty?
			lines = data.split("\n")
			...
			pbMessage(get_poke_updater_text('NO_NEW_VERSION_OR_INTERNET'))
			return
		end
	end
end
