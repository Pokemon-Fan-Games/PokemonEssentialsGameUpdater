...
```ruby
PokeUpdater.check_for_updates
```

If it is needed that the version is validated at any other point in your game, you can add this `PokeUpdater.check_for_updates` call where required.
...
| `FORCE_UPDATE`         | If this flag is sent to true and the user chooses not to update the game when prompted the a message will be shown informing that the update is requiered and the game will close.     | Upper or lower case:<br>`true`/`y`/`si`/`yes`/`s`<br>`false`/`n`/`no`                                |
